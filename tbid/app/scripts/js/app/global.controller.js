'use strict';


angular.module('tbidApp')
       .controller('GlobalCtrl', ["$scope", "$location", "$timeout","modalService","$rootScope","createPostService","$window","myProfileService","$cookieStore","$route","notificationService","$interval","postService","preLoginService",'loginService','$filter',
                                  function ($scope, $location, $timeout,modalService,$rootScope,createPostService,$window,myProfileService,$cookieStore,$route,notificationService,$interval,postService,preLoginService,loginService,$filter) 
{
    var hideLoaderTime = 500;
    $scope.rootData = {};
    $scope.rootData.error = "";
    $scope.rootData.showLogin = true;
    $scope.rootData.showRegister = true;
    $scope.rootData.isLoggedIn =false;
    $scope.rootData.showLoader = true;
    $scope.showErrorMessageOnMainScreen	= false;
    $scope.showSuccessMessageOnMainScreen = false;
    $scope.showUserDetailsTopBar = true;
    $scope.showTopProfileInfo = true;
    $rootScope.showSearchBoxFlag=true;
    
	$scope.reloadPage = function(){

	    $scope.showErrorMessageOnMainScreen	= false;
	    $scope.showSuccessMessageOnMainScreen = false;
	    $scope.showTopProfileInfo = true;
	    $scope.showLoader();
		//$route.reload();
		$location.path('/');
		$scope.hideLoader();
	};


	$rootScope.getProfileInformation = function(){

	var sendUserName = preLoginService.loggedInUserName;
    loginService.getProfileInformation(sendUserName).then(function(dataResponse){

		   $rootScope.userData = {};
		   $rootScope.userData = dataResponse.data;
		   $rootScope.userData.loggedInId = $rootScope.userData.id;
		   $rootScope.loggedInId = $rootScope.userData.id;
		   $rootScope.userData.loggedInUserName = $rootScope.userData.username
		   $rootScope.userData.loggedInUserId = $rootScope.userData.userId;
		   
		 //CLIENT SIDE HARDCODED IMG PATH - For Stubbed Mode
		   $scope.userProfilePictureImage = "data:image/jpeg;base64," ;

		 //  $rootScope.userData.profileImagePath = $scope.userProfilePictureImage //CLIENT SIDE HARDCODED IMG PATH
		   $rootScope.userData.profileImagePath = $rootScope.createUrlForImage(dataResponse.data.avatarImgPath); // WORKS WITH SERVER SIDE


		   if($rootScope.isStubbedMode){

			     $rootScope.userData.profileImagePath = $scope.userProfilePictureImage //CLIENT SIDE HARDCODED IMG PATH
		   }

		   myProfileService.userProfilePictureImage = $rootScope.userData.profileImagePath;
		   myProfileService.isLoggedIn = true;
		   
		   $scope.notificationCount = $rootScope.userData.notificationCount;
		   $scope.notificationCountReadStatus = $rootScope.userData.read;
		   //$scope.notificationCount = myProfileService.notificationCount;
		   //$scope.notificationCountReadStatus = myProfileService.notificationCountReadStatus;
	   });

	};

	$rootScope.createUrlForImage = function(avatarImgPath){
		var imageUrl = '';
		imageUrl = $location.protocol() + "://" + $location.host() +":"+ $location.port() + avatarImgPath;
		return imageUrl;
	}


    $scope.logout = function(){

      $scope.showLoader();
      
      var userId = $rootScope.userData.loggedInId;
      
      loginService.doLogout(userId).then(
		        function(){
		          $scope.rootData.isLoggedIn = false;
		          
		          $scope.showErrorMessageOnMainScreen	= false;
		          $scope.showSuccessMessageOnMainScreen = false;
		          $scope.$broadcast("logout");
		          $scope.hideLoader();
		        },
			        function(){
			          $scope.rootData.isLoggedIn = false;
			          $scope.$broadcast("logout");
			          $scope.hideLoader();
			        }
      );
    };

    $scope.$on('$locationChangeStart', function (){
      $scope.rootData.showLoader = true;
    });
    $scope.$on('$locationChangeSuccess', function () {
      $scope.rootData.showLogin = true;
      $scope.rootData.showRegister = true;
        switch($location.path()){
	          case "/register":
	            $scope.rootData.showRegister = false;
	            break;
	          case "/login":
	            $scope.rootData.showLogin = false;
	            break;
        }

        $scope.hideLoader();
    });

    $scope.showLoader = function(){
      $scope.rootData.showLoader = true;
    };

    $scope.hideLoader = function(){
      $timeout(
        function(){
          $scope.rootData.showLoader = false;
        }, hideLoaderTime);
    };

    $scope.hideLoaderNow = function(){
        $scope.rootData.showLoader = false;
    };

    $scope.$on("showLoader", function(event,data){
    	 $scope.showLoader();
    });

    $scope.$on("hideLoader", function(event,data){
    	 $scope.hideLoader();
    });

    $scope.$on("showErrorMessageOnMainScreen", function(event,data){
    	$scope.showErrorMessageOnMainScreen	= true;
    	$scope.error= data;
    	 $scope.hideLoader();
    });

    $scope.$on("showSuccessMessageOnMainScreen", function(event,data){
    	$scope.showSuccessMessageOnMainScreen	= true;
    	$scope.success= data;
    	 $scope.hideLoader();
    });

    $scope.$on("loadModalService", function(event,data){

    });

    $scope.hideLoader();

    /*This code was added as part of the Implementation of User Profile Page - START */
    $scope.isCreatingPost = false;
    $scope.toggleExpansion = function(postId) {

      if(!!$scope.isCreatingPost) {
        $scope.togglePostCreation();
      }
      if($scope.expandedPost==postId) {
        $scope.expandedPost = -1;
      } else {
        $scope.expandedPost = postId;
      }
    };

    $scope.showrepliesfunc = function(replyObj){
    	$scope.showreplies = true;
    };

    $scope.callProfileInfoCall = function(){
    	$scope.showUserDetailsTopBar = false;
    	 $scope.showTopProfileInfo = false;
    	$location.path("/profileInfo");
    };
    /*This code was added as part of the Implementation of User Profile Page - END */ 


    $rootScope.clearFlagsOnPageLoad = function(){
    	$scope.showErrorMessageOnMainScreen = false;
    	$scope.showSuccessMessageOnMainScreen = false;
		$scope.postServerError = "";
		$scope.showPostServerError = false;
    };

    $scope.callChangePassword = function(url){
    	 $rootScope.showSearchBoxFlag=false;
    	$location.path(url);
    };

    $scope.callChangeAvatar = function(url){
    	 $rootScope.showSearchBoxFlag=false;
    	$location.path(url);
    };
    
    
    
    
    $scope.notificationCount = 0;
    
	
    $scope.displayNotifications = function(){
    	$scope.notificationDisplayObj = myProfileService.notificationDisplayObj;
    };
    
    
    /*$scope.showNotificationsGlobal = function(){
         	var userId = $rootScope.loggedInId;
         	notificationService.fetchNotifications(userId).then(function(response){	
    		$scope.notificationDisplayObj = notificationService.transformNotifications(response); 		
    	
    	},function(error){
        		console.error("There was some issue fetching the notifications");
        		$scope.notificationDisplayObj = [];
        		$scope.notificationCount = 0;
        			
        });

    };*/

    $scope.sortByRecentTime = function(notif){
          	var date = new Date(notif.notifiedTime);
            return date;
    };
    
    /* When the user clicks on the button/screen, toggle between hiding and showing the dropdown content */
    $scope.showNotificationList = function () {
        	document.getElementById("myDropdown").classList.toggle("showglobal");
        	if(document.getElementById("myDropdown").classList.contains("showglobal")){
        		document.body.addEventListener('click', function(){   			
        			   if(!(document.getElementById("myDropdown") === null || document.getElementById("myDropdown") === undefined)) {
        					document.getElementById("myDropdown").classList.remove("showglobal");
        				}        				
        		}, true);
        	}
    };
    
    $scope.updateNotificationCountRead = function(){
      	var userId = $rootScope.loggedInId;
     	notificationService.updateNotificationCountRead(userId).then(function(response){	
    		//$scope.notificationCountReadStatus = true;
     		$scope.notificationCount = 0;
    	},function(error){
    		console.error("Failure while updating notification count read");
    		//$scope.notificationCountReadStatus = false;		
        	$scope.showErrorMessageOnMainScreen	= true;
        	$scope.error= "Sorry for the inconvenience.There was an issue while opening your notification list.";
        	$scope.hideLoader();
        });
    };
    
    $scope.showMoreNotifications = function(){
    	$rootScope.showSearchBoxFlag=false;
    	$location.path("/notificationListDisplay");
    	
    };
    
    $scope.checkEnterPressed = function(event,showSearchPostsTypes){
    	if(event.keyCode == 13){
    		if(showSearchPostsTypes === false){
    			$scope.showSearchPostsTypes = true;
    			$rootScope.clearFlagsOnPageLoad();
    		}
    		else if(showSearchPostsTypes === true){
    			$scope.showSearchPostsTypes=false;
    			$scope.searchQuery='';
    			$scope.showMessageNotFoundFlag=false;
    			$rootScope.clearFlagsOnPageLoad();
    		}
    	}
    };
    
    $scope.switchOnTypesCategories = function(){
    	 $scope.$broadcast("switchOnTypesCategoriesCall");
    };
    
    $(document).ready(function(){
        $('[data-toggle="tooltip"]').tooltip();   
    });
    
  }]);
