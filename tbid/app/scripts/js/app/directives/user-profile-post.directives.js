...
angular.module('tbidApp')
  .directive('userprofilePost',['IMG_PATH','$rootScope','$timeout','postService','$filter','myProfileService','loginService','preLoginService',function (IMG_PATH,$rootScope,$timeout,postService,$filter,myProfileService,loginService,preLoginService) {
    ...
    return {
      ...
      templateUrl: 'views/templates/userprofilepost.html',
    	restrict: 'E',
    			...
    			$scope.downVotedTag = function (postId,title) {
    					...
    					modalService.showModal({}, modalOptions).then(function (result) {
    							...
    							createPostService.reportPost(packageData).then(function(response){
    								...
    								//console.error("##inside user-profile-post-directives.js -- loadModalService -- REPORTING TO MODERATOR FAILED.");
    								console.error(error);
    							});
    					});
    			};
      ...
      link: function postLink(scope, element, attrs) {
	        ...
	        scope.doPushUpVote = function(postId,voteCount,index){
	            ...
	            postService.doPushUpVote(dataToServer).then(function(response){
			          	  ...
			          	  //console.error("XXXXXX: ##inside user-profile-post-directives.js directive -- doPushUpVote method -- in ERROR response section");
			        	  //console.error(error);
	         ...
	         scope.doUpVote = function(postId,voteCount){
		            ...
		            postService.doUpVoteUpdate(dataToServer).then(function(response){
			        	  ...
			        	  //console.error("XXXXXX: ##inside user-profile-post-directives.js directive -- doUpVote method -- in ERROR response section");
			        	  //console.error(error);
	         ...
	         scope.doDownVote = function(postId,voteCount){
		            ...
		            postService.doDownVoteUpdate(dataToServer).then(function(response){
		            	 ...
		            	 //console.error("XXXXXX : ##inside user-profile-post-directives.js directive -- doDownVote method -- in error response section");
		            	 //console.error(error);
	         ...
	         scope.doCommentUpVote = function(postId,commentId,index){
			        	...
			        	postService.doCommentUpVoteUpdate(dataToServer).then(function(response){
			        			...
			        			//console.error("XXXXXX : ##inside user-profile-post-directives.js directive -- doCommentUpVote method -- in error response section");
			        			//console.error(error);
	        ...
	        scope.doCommentDownVote = function(postId,commentId,index){
		        	...
		        	postService.doCommentDownVoteUpdate(dataToServer).then(function(response){		        	 	
			         	 ...
			         	 //console.error("XXXXXX : ##inside user-profile-post-directives.js directive -- doCommentDownVote method -- in error response section");
			        	 //console.error(error);
